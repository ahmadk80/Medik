...
using Medik.ServerModel.Helpers;//for appointmenthelpers
using Medik.Core.Configuration;
using Medik.Security;
using Medik.Core;
using Medik.Core.Parameters;
            ...
            try
            {
                    ...
                    if (providerId == 0)
                    {
                        var apptListforall = (from app in dbContextAppointment.Appointments
                                              ...
                                              select new
                                              {
                                                  ...
                                                  //adding appt visit type
                                                  AppointmentVisitTypeId = app.AppointmentVisitTypeId,
                                                  AppointmentVisitTypeName = dbContextAppointment.AppointmentVisitTypes.Where(a => a.AppointmentVisitTypeId == app.AppointmentVisitTypeId).Select(x => x.AppointmentVisitTypeName).DefaultIfEmpty().FirstOrDefault(),
                                                  AppointmentVisitTypeLength = dbContextAppointment.AppointmentVisitTypes.Where(a => a.AppointmentVisitTypeId == app.AppointmentVisitTypeId).Select(x => (int?)x.AppointmentVisitTypeLength).DefaultIfEmpty().FirstOrDefault()
                ...
                else if (reqType == "checkForClashingAppointment")
                {
                    ...
                    List<AppointmentModel> patAppointmentList = (from app in dbContextAppointment.Appointments
                                                                 ...
                                                                 && app.AppointmentId != apptId
                                                                 //#23: Take into conisidration the cancelled visit when validating new appt
                ...
                else if (reqType == "get-appointment-list")
                {
                    ...
                    var apptList = (from apt in dbContextAppointment.Appointments
                                    ...
                                    select new
                                    {
                                        ...
                                        FullName = apt.FirstName + " " + (String.IsNullOrEmpty(apt.MiddleName) ? " " : apt.MiddleName) + " " + apt.LastName,
                                        ...
                                        AppointmentVisitTypeId = apt.AppointmentVisitTypeId,
                                        AppointmentVisitTypeName = dbContextAppointment.AppointmentVisitTypes.Where(a => a.AppointmentVisitTypeId == apt.AppointmentVisitTypeId).Select(x => x.AppointmentVisitTypeName).FirstOrDefault(),
                                        AppointmentVisitTypeLength = dbContextAppointment.AppointmentVisitTypes.Where(a => a.AppointmentVisitTypeId == apt.AppointmentVisitTypeId).Select(x => (int?)x.AppointmentVisitTypeLength).DefaultIfEmpty().FirstOrDefault()
                                    }).ToList();
                ...
                //getting appointment visit type list 
                else if (reqType == "visittype")
                {

                    var apptVisitTypeList = dbContextAppointment.AppointmentVisitTypes.Select(x => x).ToList();
                    responseData.Status = "OK";
                    responseData.Results = apptVisitTypeList;
