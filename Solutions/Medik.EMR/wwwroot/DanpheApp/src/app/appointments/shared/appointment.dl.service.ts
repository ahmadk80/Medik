...
export class AppointmentDLService {
    ...
    //gets all appointment visit types.
  public GetAppointmentVisitType() {
    return this.http.get<any>("/api/Appointment?reqType=get-visit-doctors", this.options);
  }
  ...
  public GetAppointmentList(fromDate, toDate, providerid, status: string) {
    ...
    // getting the Appointment Visit Types
    public getAppointmentVisitTypes() {
        return this.http.get<any>("/api/Appointment?reqType=visittype");
    }
  ...
  public CheckForClashingAppointment(patientId: number, apptDate: string, providerId: number,apptId:number) {
    return this.http.get<any>("/api/Appointment?&reqType=checkForClashingAppointment"
      + '&patientId=' + patientId + '&requestDate=' + apptDate + '&providerId=' + providerId+'&apptId='+apptId);
  }
