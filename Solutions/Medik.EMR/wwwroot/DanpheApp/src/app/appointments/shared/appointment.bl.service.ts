...
export class AppointmentBLService {
  ...
  public CheckForClashingAppointment(patientId: number, apptDate: string, providerId: number,apptId:number) {
    if (patientId) {
      return this.appointmentDLService.CheckForClashingAppointment(patientId, apptDate, providerId,apptId)
        .map(res => res);
    }
    else {
      return this.appointmentDLService.CheckForClashingAppointment(0, apptDate, providerId,0)
        .map(res => res);
    }
  }
  ...
  public LoadAppointmentList(fromDate, toDate, providerid) {
    ...
    // getting Appointment Visit Types
    public getAppointmentVisitTypes() {
        return this.appointmentDLService.getAppointmentVisitTypes()
            .map(res => { return res })
