...
export class Appointment {
  ...
  public AppointmentVisitTypeId: number = null;
  public AppointmentVisitTypeName: string = null;
  ...
  public IsValidSelPatient: boolean = true;
  public IsValidSelVisitType: boolean = true;
  public IsValidTypedPatient: boolean = true;
  ...
  constructor() {
    ...
    this.AppointmentValidator = _formBuilder.group({
      ...
      'AppointmentEndTime': ['', Validators.compose([])],
      ...
      //'Duration': ['', Validators.compose([this.durationValidator])],
       //'AppointmentVisitTypeName': ['', Validators.compose([Validators.required])],
  ...
  //validate start and end time that the duration should be more than 15 minutes
  public IsValidAppointmentStartTime():boolean {
    let startTime = this.AppointmentValidator.controls["AppointmentTime"].value;
    let endTime = this.AppointmentValidator.controls["AppointmentEndTime"].value;
    if (startTime != null && endTime != null) {
        let start = moment(startTime, "HH:mm a");
        let end = moment(endTime, "HH:mm a");
        var duration = moment.duration(end.diff(start));
        var minutes = parseFloat(duration.asMinutes().toFixed(2));
        if (minutes < 15) {
            return false
        }
        else {
