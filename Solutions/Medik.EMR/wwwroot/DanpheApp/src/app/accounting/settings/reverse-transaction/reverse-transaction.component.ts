    ...
    this.settingsBLService.GetApplicationList()
      .subscribe(res => {
        if (res.Status == 'OK') {
          ...
          let sList = this.accountingService.accCacheData.Sections.filter(sec => sec.SectionId != 4); // 4 is Manual_Voucher (FIXED for Medik) //mumbai-team-june2021-danphe-accounting-cache-change
          sList.forEach(s => {
